...
# -------------------------
# Load PDF content and embeddings
# -------------------------
all_chunks = []
chunk_mapping = []
...
with open("data/pdf_chunks.json", "w", encoding="utf-8") as f:
    json.dump([{"keyword": k, "chunk": c} for k, c in zip(chunk_mapping, all_chunks)],
...
emb_chunks = model.encode(all_chunks, convert_to_tensor=True, show_progress_bar=True)
